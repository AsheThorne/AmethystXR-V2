...
public:
    ...
    void setUIImageResourcesPreloadCount(uint32_t imageCount);
    ...
    /// Get the UI image resources preload count
    //// @returns The UI image resources preload count
    [[nodiscard]] uint32_t getUIImageResourcesPreloadCount() const;
...
private:
    ...
    // ----------------------------------------- //
    std::string m_Name;
    AxrAssetCollection m_AssetCollection = AxrAssetCollection(false, AXR_GRAPHICS_API_UNDEFINED);
    entt::registry m_Registry;
    AxrEntityConst_T m_MainCamera = AxrEntityConst_T(m_Registry, entt::null);
    uint32_t m_UIImageResourcesPreloadCount = 0;
